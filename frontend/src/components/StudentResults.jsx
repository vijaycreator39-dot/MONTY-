...
const StudentResults = () => {
  ...
  return (
    <section id="results" className="relative py-16 sm:py-24 overflow-hidden">
      ...
      <div className="relative">
        ...
        >
          {items.map((t, idx) => {
            ...
            const isPlaying = playing === idx && hasVideo;
            return (
              <div
                key={idx}
                data-card
                ...
                <div className="relative h-[340px] sm:h-[400px] bg-gradient-to-br from-[#2a0812] to-[#4a1024] overflow-hidden">
                  {isPlaying ? (
                    ...
                    <>
                      ...
                      />
                      <button
                        onClick={() => hasVideo && setPlaying(idx)}
                      ...
                      <span className="absolute top-3 left-3 bg-black/70 backdrop-blur text-white text-[9px] sm:text-[10px] tracking-wider uppercase px-2 sm:px-2.5 py-1 rounded-full flex items-center gap-1">
                        {hasVideo ? 'Tap to play' : (<><Clock size={10} /> Video coming soon</>)}
                      </span>
                      {t.badge && (
      ...
      <p className="text-center mt-5 sm:mt-6 text-[12px] sm:text-[13px] text-[#1a0d12]/50 px-5">
        Swipe or use arrows to navigate · video links will be added soon
      </p>
    </section>
  );
};
